...
class ComputeSessionPerformanceUseCase(
    ...
    suspend operator fun invoke(
        ...
        for (answer in answers) {
            ...
            val pQuestion = when (attempt.mode) {
                AttemptMode.GAME -> {
                    val speedScore = answer.timeLimitMs
                        ?.takeIf { it > 0 }
                        ?.let { limit ->
                            (1f - (answer.timeMs.toFloat() / limit).coerceIn(0f, 1f)).coerceIn(0f, 1f)
                        }
                        ?: if (answer.isCorrect) 0.5f else 0f
                    val base = difficultyWeight * ((0.70f * if (answer.isCorrect) 1f else 0f) + (0.30f * speedScore))
                    if (answer.isCorrect) base else base - (0.10f * difficultyWeight)
                }

                AttemptMode.STUDY -> difficultyWeight * (0.85f * if (answer.isCorrect) 1f else 0f) * 0.35f
            }.coerceIn(0f, 1.2f)
    ...
    private fun difficultyWeight(difficulty: DifficultyLevel): Float = when (difficulty) {
