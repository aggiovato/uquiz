...
class UpdateQuestionStatsFromAttemptUseCase(
    ...
    suspend operator fun invoke(
        ...
        answers.forEach { answer ->
            ...
            val studyCorrect =
                (current?.studyCorrect ?: 0) + if (attempt.mode == AttemptMode.STUDY && answer.isCorrect) 1 else 0
            val gameAttempts = (current?.gameAttempts ?: 0) + if (attempt.mode == AttemptMode.GAME) 1 else 0
            val gameCorrect =
                (current?.gameCorrect ?: 0) + if (attempt.mode == AttemptMode.GAME && answer.isCorrect) 1 else 0
            ...
            val avgGameTimeMs =
                if (attempt.mode == AttemptMode.GAME) {
