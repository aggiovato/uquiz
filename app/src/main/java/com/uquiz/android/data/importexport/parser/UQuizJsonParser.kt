...
class UQuizJsonParser(
    ...
    private fun UQuizFolderDto.toImportNode(): ImportFolderNode =
        ImportFolderNode(
            name = name.trim(),
            colorHex = color?.takeIf { it.isNotBlank() } ?: ContentDefaults.DefaultFolderColorHex,
            icon = icon?.takeIf { it.isNotBlank() } ?: ContentDefaults.DefaultFolderIconKey,
    ...
    private fun UQuizPackDto.toImportNode(): ImportPackNode =
        ImportPackNode(
            ...
            colorHex = color?.takeIf { it.isNotBlank() } ?: ContentDefaults.DefaultPackColorHex,
            icon = icon?.takeIf { it.isNotBlank() } ?: ContentDefaults.DefaultPackIconKey,
            questions = questions.map { question ->
                ImportQuestionNode(
                    text = question.text.trim(),
                    explanation = question.explanation?.trim()?.takeIf { it.isNotBlank() },
                    difficulty = DifficultyLevel.entries.firstOrNull {
                        it.name.equals(question.difficulty, ignoreCase = true)
                    } ?: DifficultyLevel.MEDIUM,
                    options = question.options.sortedBy { it.position }.map { option ->
                        ImportOptionNode(
